...
@section('content')
    ...
    <section class="content">
        <div class="box box-info">
            ...
            <div class="box-body">
                <form class="form-horizontal" action="{{ route('pengumuman.update', $pengumuman->id) }}" method="POST"
                            ...
                            <select class="form-control select2 @error('kategori_id') is-invalid @enderror" id="kategori_id"
                                ...
                                @foreach ($kategoris as $kategori)
                                    <option value="{{ $kategori->id }}"
                                        @if (old('kategori_id', $pengumuman->kategori_id) == $kategori->id) selected="selected" @endif> {{ $kategori->name }}
                                    </option>
                                @endforeach
                            </select>
                            @error('kategori_id')
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control @error('judul') is-invalid @enderror" id="judul"
                                name="judul" value="{{ old('judul', $pengumuman->judul) }}" placeholder="Judul">
                            @error('judul')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <textarea class="tinymce @error('body') is-invalid @enderror" rows="10" name="body" id="body">{{ old('body', $pengumuman->body) }}</textarea>
                            @error('body')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                        ...
                        <div class="col-sm-10">
                            ...
                            @if ($pengumuman->gambar)
                                <img src="{{ asset('storage/' . $pengumuman->gambar) }}"
                                    class="img-fluid img-thumbnail mb-3 col-sm-3 d-block" alt="Gambar Artikel"
                                    width="50%">
                            @else
                                <img class="img-fluid img-thumbnail mb-3 col-sm-5">
                            @endif
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <select class="form-control @error('status') is-invalid @enderror" id="status"
                                name="status">
                                <option value="" hidden>-- Choose --</option>
                                <option value="0" @if (old('status', $pengumuman->status) == 0) selected="selected" @endif> Private
                                </option>
                                <option value="1" @if (old('status', $pengumuman->status) == 1) selected="selected" @endif> Publish
                                </option>
                            </select>
                            @error('status')
                                <div class="invalid-feedback">
